...
export async function uploadDocuments() {
  try {
    for (const doc of documents) {
      ...
      if (doc.content) {
        doc.vector = await getEmbedding(doc.KB);
      }
console.log(`Uploading document with ID: ${doc.id}, ${doc.vector}`);
      const existing = await searchClient.getDocument(doc.id).catch(() => null);
      ...
      if (existing) {
        ...
        console.log(`Uploaded new document with ID: ${doc.id}`);
      }
    }
  } catch (error) {
